...
classes.forEach(cls => {
    ...
    cls.properties = cls.properties
        ...
        .map(it => ({
            name: it._name,
            type: formatType(it.type, { isProperty: true, isSettableProperty: !it.readOnly }, it.values),
...
function formatType(type, options, values = '') {
    return ': ' + parseType(type, options, values);
}

function parseType(type, options, values = '') {
