/*!
 * Paper.js v0.12.15-undefined - The Swiss Army Knife of Vector Graphics Scripting.
 * http://paperjs.org/
 *
			...
			if (children) {
				...
				for (var i = 0, l = children.length; i < l; i++) {
					...
					if (isScaling) {
						...
						console.log(translation.x, scaling.x, translation.x/scaling.x)
						...
						switch (horizontal) {
							case "scale":
								...
								break;
							case "center":
								mx.translate(diff.x / 2 * (left ? 1 : -1), 0).scale(flipped.x, 1, this._constraintsPivot);
								break;
							case "end":
								mx.translate(left ? diff.x : 0, 0).scale(flipped.x, 1,  this._constraintsPivot);
								break;
							default:
								mx.translate(!left ? -diff.x : 0, 0).scale(flipped.x, 1,  this._constraintsPivot);
								break;
						...
						switch (vertical) {
							...
							case "center":
								mx.translate(0, diff.y / 2 * (top ? 1 : -1)).scale(1, flipped.y, this._constraintsPivot);
								break;
							case "end":
								mx.translate(0, top ? diff.y : 0).scale(1, flipped.y,  this._constraintsPivot);
								break;
							default:
								mx.translate(0, !top ? -diff.y : 0).scale(1, flipped.y, this._constraintsPivot);
						}
...
var SelectorInfo = Item.extend(
	 {
		...
		initialize: function SelectorInfo(content, point, corner) {
		...
		_initialize: function (content) {
			this.setLabel(
				new PointText({
					fillColor:this._project._selector.fillColor,
					point: [0, 0],
					content: content,
					fontSize: 12,
					insert: false,
			...
			this.setBackground(
				new Shape.Rectangle({
					fillColor: this._project._selector.strokeColor,
					insert: false,
...
var DrawControlEvent = Event.extend({
...
var Tool = PaperScopeItem.extend(
	 {
		...
		getActived: function () {
			return this._scope.tool === this && !this._paused;
		},
