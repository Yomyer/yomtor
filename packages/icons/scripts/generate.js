...
const generateIcons = async (files) => {
  ...
  for (let x = 0; x < files.length; x++) {
    ...
    const d = Array.from(svg.matchAll(/<path.*\sd="([^"]*)"/g), (m) => m[1])[0]
    ...
    names.push(
      StringUtility.toCase(path.parse(file).name, CaseConverterEnum.PascalCase)
    )
