...
export const TreeView: React.FC<TreeViewProps> = ({
  ...
  useEffect(() => {
    if (target) {
      ...
      if (lineRef.current) {
        ...
        lineRef.current.style.left = `${indentWitdh * (depths[current] + 1)}px`
      }
      ...
      >
        {virtualItems.map((item) => {
          ...
          return (
            ...
            >
              <Droppable
                onMove={(event) => dropMoveHandler(event, item.index)}
                onDrop={dropHandler}
                disabled={!sortable}
              >
                {() => (
                  <Draggable
                    phantom={draggable}
                    move={draggable}
                    onMouseDown={(event) => mouseDownHandler(event, item)}
                    ...
                    >
                      <div className={classes.indents}>
                        ...
                        <em
                          ...
                          onClick={(event) => collapser(node, event)}
                          onMouseDown={(event) => {
                        ...
                        >
                          <PlayIcon
                            rotate={
                              (!isUndefined(node[collapsedProp])
                                ? !node[collapsedProp]
                                : !collapsed) && 90
                            }
                          />
  ...
  return (
    <div {...props} className={classes.root} onMouseLeave={mouseLeaveHandler}>
